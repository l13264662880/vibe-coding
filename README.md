...
一个从零搭起来的网站项目。**当前进度：Day 8 → mock 数据版主视图已跑起来**。
...
| 文件 | 归属 | 作用 |
| --- | --- | --- |
| `index.html` | Day 7 / Day 8 | 页面结构：标题、第一件事高亮区、输入框、任务列表、四种状态（加载/有数据/空/出错）容器、撤销条 |
| `style.css` | Day 7 / Day 8 | 全部样式：瓷灰底 + 墨色宋体 + 竹影背景；Day 8 补加载中/出错状态样式 |
| `app.js` | Day 7 / Day 8 | 全部逻辑：增删改、拖拽排序、完成/撤销、localStorage 读写；Day 8 抽「数据源」层 + 四种状态 |
| `mock-data.js` | Day 8 | 本地假数据源：返回写死的示例任务，模拟异步加载，可切 content/empty/error 三种场景 |
